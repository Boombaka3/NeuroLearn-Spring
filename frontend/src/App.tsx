...
import type { PreCourseEvaluationAttempt } from './types/preCourseEvaluation'
...
function App() {
  ...
  useEffect(() => {
    dispatch(hydratePreCourseEvaluation(loadPreCourseEvaluationAttempt() as PreCourseEvaluationAttempt | null))
  }, [dispatch])

  const startCourse = () => {
    if (!preCourseHydrated) {
      const preCourseAttempt = loadPreCourseEvaluationAttempt() as PreCourseEvaluationAttempt | null
