...
import { areKnowledgeQuestionsComplete, areLikertQuestionsComplete } from './lib/courseEvaluationLogic'
...
export default function CourseEvaluationPage({ onBack, onContinue }) {
  ...
  const results = attempt.completedAt && Number.isFinite(attempt.score)
    ? {
        score: attempt.score,
  ...
  return (
    <div className="ce-page">
      <div className="ce-shell">
        ...
        {currentStep === 'results' && !results && (
          <section className="ce-panel" aria-labelledby="results-heading">
            <div className="ce-panel-head">
              <h2 id="results-heading" ref={resultsHeadingRef} tabIndex={-1}>{t('postEval.results.title')}</h2>
              <p className="ce-inline-error" role="alert">
                {attempt.remoteSubmissionError || t('postEval.results.savedError')}
              </p>
            </div>
            <div className="ce-actions">
              ...
              >
                {isRetryingUpload ? t('postEval.results.retrying') : t('postEval.results.retrySave')}
              </button>
            </div>
          </section>
        )}
